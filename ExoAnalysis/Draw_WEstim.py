...
#    ncat=
    ...
    Data.Rebin(RB_)
    print "--> error one data=", Data.GetBinError(1)
    ...
    TT.Rebin(RB_)
    print "--> error one tt=", TT.GetBinError(1)
    VV=file.Get(categoriy).Get("VV")
    VV.Rebin(RB_)
    print "--> error one vv=", VV.GetBinError(1)
    #W.Add(VV)
    SingleT=file.Get(categoriy).Get("SingleTop")
    SingleT.Rebin(RB_)
    print "--> error one sT=", SingleT.GetBinError(1)
    DYS=file.Get(categoriy).Get("ZTT")
    DYS.Rebin(RB_)
    print "--> error one ztt=", DYS.GetBinError(1)
