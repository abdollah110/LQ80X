...
//                        bool ElePtCut = tauPt->at(iele) > TauPtCut_  && fabs(tauEta->at(iele)) < 2.3 ;
//                        bool TauIdIso =  taupfTausDiscriminationByDecayModeFinding->at(iele) > 0.5 && tauByTightMuonRejection3->at(iele) > 0 && tauByMVA6LooseElectronRejection->at(iele) > 0 && fabs(tauDxy->at(iele)) < 0.05;
...
//                        LepCor=getCorrFactorMuon74X(isData,  muPt->at(imu), muEta->at(imu) , HistoMuId,HistoMuIso,HistoMuTrg);
                        ...
                        TLorentzVector Mu4Momentum, ele4Momentum, Z4Momentum, Jet4Momentum,ExtraMu4Momentum, ExtraEle4Momentum,KJet4Momentum;
...
//                        float CLoseJetTauPt=elePt->at(iele);
//                        float CLoseJetTauEta=eleEta->at(iele);
...
//                                    if (Refer_R_jettau < 0.5 && jetPt->at(kjet)  >= elePt->at(iele)) {
...
//                            extraElectronExist= true;
                        ...
                        bool  GeneralMuEleSelection=  !extraMuonExist && !extraElectronExist && MuPtCut && elePtCut && MuIdIso && eleIdIso && Mu4Momentum.DeltaR(ele4Momentum) > 0.5;
                        cout<<event<< "  " <<(!extraMuonExist) << (!extraElectronExist) << (MuPtCut) << (elePtCut) << (MuIdIso) << (eleIdIso) << (Mu4Momentum.DeltaR(ele4Momentum) > 0.5)<<"\t"<< TotalWeight * LepCor<< " "<<LumiWeight <<" "<< GetGenWeight <<" "<< PUWeight <<" "<< TopPtReweighting <<"  1.0 " <<GenTopPt<<" "<< GenAntiTopPt<<"\n"  ;
...
//                                cout <<"jetPt= "<< jetPt->at(ijet)<<"   jetRawPt= "<< jetRawPt->at(ijet) <<"  jetEn= "<< jetEn->at(ijet)<<"   jetRawEn= "<< jetRawEn->at(ijet) << "  jetJECUnc="<<jetJECUnc->at(ijet)<<"\n";
                            ...
                            if (jetPFLooseId->at(ijet) > 0.5 && jetPt->at(ijet) > BJetPtCut && fabs(jetEta->at(ijet)) < 2.4 && Jet4Momentum.DeltaR(ele4Momentum) > 0.5 && Jet4Momentum.DeltaR(Mu4Momentum) > 0.5 && jetpfCombinedInclusiveSecondaryVertexV2BJetTags->at(ijet) >  LooseCSV ){
...
//                            }
                        ...
                        //###############################################################################################
                        //###############################################################################################
                        bool ElePassIsolation= IsoEle < LeptonIsoCut ;
//                        bool ElePassIsolation= tauByLooseIsolationMVArun2v1DBoldDMwLT->at(iele) > 0.5 ;
                        bool LepPassIsolation= IsoMu < LeptonIsoCut;
                        
                        const int size_isoCat = 9;
                        bool Isolation = ElePassIsolation && LepPassIsolation;
                        bool AntiIsolation = !ElePassIsolation && !LepPassIsolation;
                        bool TauIsoLepAntiIso = ElePassIsolation && !LepPassIsolation;
                        bool TauAntiIsoLepIso = !ElePassIsolation && LepPassIsolation;
                        bool TauIso = ElePassIsolation;
                        bool TauAntiIso = !ElePassIsolation;
                        ...
                        bool MT_category[size_mTCat] = {NoMT,LoWMT};
                        std::string MT_Cat[size_mTCat] = {"","_LowMT"};
...
//                        std::string MT_Cat[size_mTCat] = {""};
                        ...
                        bool MCTrigger = (HLTEleMuX >> 26 & 1) == 1;;
                                            bool Trigger_category[size_trgCat] = {PassTrigger,MCTrigger};
                                            std::string trg_Cat[size_trgCat] = {"","_MCtrg"};
...
//                        std::string trg_Cat[size_trgCat] = {""};
                        ...
                        //###############################################################################################
                        const int size_ST = 4;
                        bool ST_category[size_ST] = {1,JetBJet_Selection,DiJet_Selection,NonBJet_Selection};
                        std::string ST_Cat[size_ST] = {"_inclusive","_JetBJet","_DiJet","_NoBJet"};
...
//                                                                    plotFill("MuEle_tmass"+FullStringName,tmass,500,0,500,TotalWeight * LepCor);
//                                                                    plotFill("MuEle_VisMass"+FullStringName,Z4Momentum.M(),500,0,500,TotalWeight * LepCor);
                                                                    plotFill("MuEle_MuPt"+FullStringName,muPt->at(imu),300,0,300,TotalWeight * LepCor);
                                                                    plotFill("MuEle_MuIso"+FullStringName,IsoMu,100,0,10,TotalWeight * LepCor);
                                                                    plotFill("MuEle_MuEta"+FullStringName,muEta->at(imu),100,-2.5,2.5,TotalWeight * LepCor);
                                                                    plotFill("MuEle_ElePt"+FullStringName,elePt->at(iele),200,0,200,TotalWeight * LepCor);
                                                                    plotFill("MuEle_EleEta"+FullStringName,eleEta->at(iele),100,-2.5,2.5,TotalWeight * LepCor);
//                                                                    plotFill("MuEle_CloseJetTauPt"+FullStringName,CLoseJetTauPt,500,0,500,TotalWeight * LepCor);
                                                                    plotFill("MuEle_NumJet"+FullStringName,JetVector.size(),10,0,10,TotalWeight * LepCor);
                                                                    plotFill("MuEle_NumBJet"+FullStringName,BJetBVector.size(),10,0,10,TotalWeight * LepCor);
//                                                                    plotFill("MuEle_nVtx"+FullStringName,nVtx,50,0,50,TotalWeight * LepCor);
//                                                                    plotFill("MuEle_nVtx_NoPU"+FullStringName,nVtx,50,0,50,TotalWeight * LepCor / PUWeight);
                                                                    plotFill("MuEle_MET"+FullStringName,pfMET,500,0,500,TotalWeight * LepCor);
//                                                                    plotFill("MuEle_M_taujet"+FullStringName,M_TauJet,1000,0,1000,TotalWeight * LepCor);
                                                                    plotFill("MuEle_LeadJetPt"+FullStringName,leadJetPt_,300,0,300,TotalWeight * LepCor);
                                                                    plotFill("MuEle_SubLeadJetPt"+FullStringName,subLeadJetPt_,300,0,300,TotalWeight * LepCor);
                                                                    plotFill("MuEle_LeadJetEta"+FullStringName,leadJetEta_,100,-2.5,2.5,TotalWeight * LepCor);
                                                                    plotFill("MuEle_SubLeadJetEta"+FullStringName,subLeadJetEta_,100,-2.5,2.5,TotalWeight * LepCor);
//                                                                    plotFill("MuEle_ST_DiJet"+FullStringName,ST_DiJet,100,0,1000,TotalWeight * LepCor);
                                                                    plotFill("MuEle_ST_MET"+FullStringName,ST_MET,100,0,1000,TotalWeight * LepCor);
