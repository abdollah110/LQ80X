...
void Skimmer::Loop(TString outputName, int skm)
{
    ...
    for (int jentry=0; jentry<nentries;jentry++) {
        ...
        for (int iele = 0; iele < nEle; ++iele){
            ...
            if (elePt->at(iele) > 49.5 && fabs(eleEta->at(iele))< 2.5 &&  IsoEle < 0.20){
                Ele4Momentum.SetPtEtaPhiM(elePt->at(iele),eleEta->at(iele),elePhi->at(iele),eleMass);
                
                for (int itau = 0; itau < nTau; ++itau){
                    if (tauPt->at(itau) > 48 && fabs(tauEta->at(itau)) < 2.3 && tauCombinedIsolationDeltaBetaCorrRaw3Hits->at(itau) < tauPt->at(itau) * 0.2 && tauByLooseMuonRejection3->at(itau) &&  tauByMVA6LooseElectronRejection->at(itau) ){
        ...
        for (int imu = 0; imu < nMu; ++imu){
            ...
            if (muPt->at(imu) > 49.5 &&  fabs(muEta->at(imu)) < 2.4 &&   muIsLooseID->at(imu) > 0  && IsoMu < 0.20 ){
